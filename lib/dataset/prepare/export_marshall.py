...
and fixed-rate (15 FPS) extraction when timestamps are not available.
"""
...
def process_files_with_timestamps(files, timestamp_df, output_dir, cam_num):
...
def process_files_at_8fps(files, output_dir, cam_num):
    ...
    for video_path in files:
        ...
        while True:
            ...
            # Process every nth frame to achieve approximately 15 FPS
...
def extract_frames_with_timestamps(input_dir, output_dir, timestamps):
    ...
    if timestamps is not None:
        ...
        timestamps_array_1 = process_files_with_timestamps(ch1_files, timestamps, output_dir, cam_num=1)
        timestamps_array_2 = process_files_with_timestamps(ch2_files, timestamps, output_dir, cam_num=2)
    else:
        # Process at 15 FPS
        print("Processing at 15 FPS (no timestamps file provided)...")
        timestamps_array_1 = process_files_at_8fps(ch1_files, output_dir, cam_num=1)
        timestamps_array_2 = process_files_at_8fps(ch2_files, output_dir, cam_num=2)
