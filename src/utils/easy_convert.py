...
def convert_rot_rep(from_rep_to_rep, rot_representation):
    """
    Converts between different rotation representations based on the input string.
    Args:
        from_rep_to_rep (str): A string specifying the conversion in the format
                            "<from_rep> -> <to_rep>", e.g., "rotation_6d -> aa".
                            Valid representations: "rotation_6d", "aa", "rotation_matrix", "quaternion".
        rot_representation (torch.Tensor): The input tensor representing the rotation in the source format.
    Returns:
        torch.Tensor: The converted rotation in the target format.
    Raises:
        ValueError: If the conversion is not supported or the format is invalid.
    """
    # Define conversion mappings
    conversion_map = {
        ("rotation_6d", "aa"): rotation_6d_to_aa,
        ("rotation_6d", "rotation_matrix"): rotation_6d_to_matrix,
        ("rotation_6d", "quaternion"): rotation_6d_to_quaternion,
        ("aa", "rotation_matrix"): axis_angle_to_matrix,
        ("aa", "rotation_6d"): aa_to_rotation_6d,
        ("aa", "quaternion"): axis_angle_to_quaternion,
        ("rotation_matrix", "rotation_6d"): matrix_to_rotation_6d,
        ("rotation_matrix", "aa"): matrix_to_axis_angle,
        ("rotation_matrix", "quaternion"): matrix_to_quaternion,
        ("quaternion", "aa"): quaternion_to_axis_angle,
        ("quaternion", "rotation_matrix"): quaternion_to_matrix,
        ("quaternion", "rotation_6d"): quaternion_to_rotation_6d,
    }

    # Parse the from_rep_to_rep argument
    try:
        from_rep, to_rep = from_rep_to_rep.split(" -> ")
    except ValueError:
        raise ValueError("Invalid format for from_rep_to_rep. Expected format is '<from_rep> -> <to_rep>'.")

    # Check if the conversion is supported
    if (from_rep, to_rep) not in conversion_map:
        raise ValueError(f"Conversion from {from_rep} to {to_rep} is not supported.")

    # Perform the conversion using the corresponding function
    conversion_function = conversion_map[(from_rep, to_rep)]
    return conversion_function(rot_representation)


def rotation_6d_to_aa(d6: torch.Tensor) -> torch.Tensor:
    """
...
def rotation_6d_to_quaternion(d6: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        d6: 6D rotation representation, of size (*, 6)
    Returns:
        Quaternion representation, of size (*, 4)
    """
    # First convert 6D to rotation matrix
    matrix = rotation_6d_to_matrix(d6)
...
def aa_to_rotation_6d(axis_angle: torch.Tensor) -> torch.Tensor:
    """
    Convert axis-angle representation to 6D rotation representation.
    Args:
        axis_angle: Axis-angle representation, of size (*, 3)
    Returns:
        6D rotation representation, of size (*, 6)
    """
    # First convert axis-angle to rotation matrix
    matrix = axis_angle_to_matrix(axis_angle)
    # Then convert rotation matrix to 6D
    return matrix_to_rotation_6d(matrix)


def quaternion_to_rotation_6d(quaternion: torch.Tensor) -> torch.Tensor:
    """
    Convert quaternion representation to 6D rotation representation.
    Args:
        quaternion: Quaternion representation, of size (*, 4)
    Returns:
        6D rotation representation, of size (*, 6)
    """
...
def matrix_of_angles(cos, sin, inv=False, dim=2):
    ...
    if dim == 2:
        ...
        row1 = torch.stack((cos, -sin, 0 * cos), dim=-1)
        row2 = torch.stack((sin, cos, 0 * cos), dim=-1)
        row3 = torch.stack((0 * sin, 0 * cos, 1 + 0 * cos), dim=-1)
...
def matrix_to_euler_angles(matrix, convention: str):
    ...
    if tait_bryan:
        if i0 is None or i2 is None:
            raise ValueError("Invalid indices: i0 and i2 must not be None.")
